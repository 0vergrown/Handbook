...
description: "Copies the target's chosen origin onto the actor's copy layer."
---
...
This is the copy-only convenience over [`origins:transfer_origin`](/docs/datapack/bientity-actions/transfer_origin): it delegates to the same origin-transfer core (which mirrors how [`apoli:transfer`](/docs/datapack/bientity-actions/transfer) moves powers by source) with `mode: steal`, `copy: true`. Reach for `transfer_origin` when you also want to **take** the origin away from the target, or hand yours to them.

Type ID: `origins:copy_origin`

> This is an Origins addon action (not core Apoli). Both the actor and the target must be players. If the target has no origin (or it's the "blank"/empty one) in `from_layer`, nothing happens.
...
- [[origins:transfer_origin](/docs/datapack/bientity-actions/transfer_origin)](/docs/datapack/bientity-actions/transfer_origin) — the full steal/give/copy origin action; `copy_origin` is its `copy: true` shorthand.
- [[apoli:transfer](/docs/datapack/bientity-actions/transfer)](/docs/datapack/bientity-actions/transfer) — the lower-level, layer-agnostic power move/copy action (`apoli:transfer`). `copy_origin` is the Origins-aware convenience built on the same source system.
