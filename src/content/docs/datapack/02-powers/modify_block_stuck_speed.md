...
> This **replaces** the block's multiplier rather than scaling it, so a value of `1.0` frees the entity completely. When several matching powers apply, the largest value on each axis wins — the least restrictive power decides.
