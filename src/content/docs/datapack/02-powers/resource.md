...
| `hud_render`     | [Hud Render](/docs/datapack/data-types/hud-render)                        | _hidden_       | Determines how the resource is visualized on the HUD. A bar needs a value to fill up to, so an uncapped resource needs a `max` on the `hud_render` — see [Hud Render](/docs/datapack/data-types/hud-render).                                                                                                                                                                                                                                                        |
...
> A bar has no scale without a `max`. If you want one on an uncapped resource, put a `max` on the `hud_render` instead — the resource stays uncapped and the bar fills up to that value.
