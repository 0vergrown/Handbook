...
------------------------------------|-------------------------------------------
  ...
  `local_horizontal_normalized`     |  Same as `local_horizontal`, except the axes are normalized — the offset keeps its full length whatever the entity's pitch is. This is a pure rotation around the entity's yaw, so unlike `local_horizontal` it stays well-defined when the entity looks straight up or straight down. *(Before Apoli 1.45.0 those two poses collapsed the result to `0`, the same as `local_horizontal`.)*  
